        ...
        if(m_data === null) 
        {
            ...
            .then((response)=>{
                ...
                m_data.left_team_players.forEach((player)=>{
                    player.pre_points = 0 
                    player.captain_selection = 0 
                    player.vicecaptain_selection = 0
                })
    ...
    return (
        <React.Fragment>
           ...
           {/* Here i am going to give option to edit and add player data */}
            ...
            <span><span style={{color:'grey',fontSize:'13px'}}> change data? </span>&nbsp;&nbsp;
                <button onClick={()=> handleEdit()} className="vp-btn btn btn-sm btn-dark">Edit Data</button>&nbsp;&nbsp;
                <button onClick={()=> handleExtraData()} className="vp-btn btn btn-sm btn-primary">Edit Extra Data</button>
