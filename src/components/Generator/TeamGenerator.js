...
let get_leverage = (final_team,c_vc_obj)=>{
    let leverage = 1; 
    for(let i=0;i<final_team.length;i++)
    {
        for(let j=0;j<final_team[i].length;j++)
        {
            let p = final_team[i][j] 
            leverage = leverage * (100/(parseFloat(p.selected_by)===0? 1 : parseFloat(p.selected_by)));
            if(c_vc_obj.captain === p.player_index)
                leverage = leverage * (100/(parseFloat(p.captain_selection)===0? 1 : parseFloat(p.captain_selection)));
            if(c_vc_obj.vicecaptain === p.player_index)
                leverage = leverage * (100/(parseFloat(p.captain_selection)===0? 1 : parseFloat(p.vicecaptain_selection)));
        }
    }
    return leverage;
}

let get_pre_points = (final_team,c_vc_obj)=>{
    let points = 0;
    for(let i=0;i<final_team.length;i++)
    {
        for(let j=0;j<final_team[i].length;j++)
        {
            ...
            if(c_vc_obj.captain === p.player_index)
                points = points + parseFloat(p.pre_points)*2
            else if(c_vc_obj.vicecaptain === p.player_index)
                points = points + parseFloat(p.pre_points)*1.5
            else 
                points = points + parseFloat(p.pre_points)
        }
    }
    return points;
} 
...
let generateTeams = (selectedPlayers,fixedPlayers,captainPlayers,vicecaptainPlayers,partisionStrategy,leftRange,rightRange,combination,tn,selectionStrategy)=>{
   ...
   // console.log(selectedPlayers)
    ...
    let try_array = [
        10000,9000,8000,7000,6000,5000,4000,3000,2000,1000,900,800,700,600,500,400,300,200,100,90,80,70,60,50,40,30,20,10,5,3,1
    ]
  //  console.log('hi')
   // console.log(selectionStrategy)
    for(let vikas =0;vikas<try_array.length;vikas++)
    {
        ...
        {
            console.log(dp)
            // sort them out based on pre points and  through that stuff out
            created_team_list.sort((x,y)=>{
                if(x.pre_points<y.pre_points)
                    return 1; 
                return -1;
            })
          //  console.log(created_team_list.length)
            if(tn>created_team_list.length)
                return created_team_list; 
            created_team_list = created_team_list.slice(0,tn)
            for(let jp=0;jp<created_team_list.length;jp++)
                created_team_list[jp].team_number = jp+1;
           // console.log(tn)
           // console.log(created_team_list)
            return created_team_list; 
        }
        if(dp>100000){
       ...
       // console.log(eligible_players)
        ...
        if(credit_valid !== -1 && partision_valid && selection_valid )
        {
            ...
            if(c_vc_obj!=null)
            {
                ...
                let leverage = get_leverage(full_team,c_vc_obj)
                let pre_points = get_pre_points(full_team,c_vc_obj)
               // console.log(leverage)
               // console.log(pre_points)
